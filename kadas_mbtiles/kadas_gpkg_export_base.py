...
class KadasGpkgExportBase(QObject):
    ...
    def write_layers(
        ...
        pyramids,
        filterExtent=None,
        filterExtentCrs=None,
        rasterExportScale=None,
    ):
        canceled = False
        for layerid in selected_layers:
            ...
            if filterExtent and filterExtentCrs and not filterExtent.isNull():
                layerFilterExtent = QgsCoordinateTransform(
                    filterExtentCrs, layer.crs(), QgsProject.instance()
                ).transformBoundingBox(filterExtent)
            ...
            if layer.type() == QgsMapLayer.VectorLayer:
                ...
                feedback.progressChanged.connect(
                    lambda prog: self.updateProgress(prog, pdialog)
                )
                ...
                saveOptions.actionOnExistingFile = (
                ...
                if ret[0] == 0:
                    ...
                    messages.append(
                        ...
                        % (
                            layer.name(),
                            self.tr("Write failed: error %d (%s)") % (ret[0], ret[1]),
                        )
                    )
                ...
                feedback.progressChanged.connect(
                    lambda prog: self.updateProgress(prog, pdialog, pyramids)
                )
                pdialog.canceled.connect(feedback.cancel)
                ...
                writer.setCreateOptions(
                    [
                        "RASTER_TABLE=%s" % self.safe_name(layer.name()),
                ...
                if rasterExportScale:
                    dpi = 90
                    cols = round(
                        exportExtent.width() / (rasterExportScale * 0.0254) * dpi
                    )
                    rows = round(
                        exportExtent.height() / (rasterExportScale * 0.0254) * dpi
                    )

                ret = writer.writeRaster(
                    pipe, cols, rows, exportExtent, provider.crs(), feedback
                )
                if ret == 0:
                    added_layer_ids.append(layerid)
                    added_layers_by_source[layer.source()] = layerid
                    ...
                    messages.append(
                        "%s: %s"
                        % (layer.name(), self.tr("Write failed: error %d") % ret)
                    )
